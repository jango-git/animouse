...
test("constructor: should sort actions by value ascending", () => {
  // Create actions in unsorted order: 0.1, -0.5, 0
  const action1 = buildMockPolarAction(1, 0.1);
  ...
  const action1ToAction2Distance = -0.5 + PI2 - 0.1;
  const action1ToTargetDistance = -1 + PI2 - 0.1;

  const action3Weight = 0;
  const action2Weight = action1ToTargetDistance / action1ToAction2Distance;
  const action1Weight = 1 - action2Weight;
  blendTree.setBlend(1, -1);

  // If sorted correctly (-0.5, 0, 0.1), action2 should have weight 1
  assertEqualWithTolerance(
    action1.action.weight,
    action1Weight,
    "action1 (azimuth 0.1) should have weight 0 when blend is at minimum",
  );
  assertEqualWithTolerance(
    action2.action.weight,
    action2Weight,
    "action2 (azimuth -0.5) should have weight 1 when blend clamps to minimum",
  );
  assertEqualWithTolerance(
    action3.action.weight,
    action3Weight,
    "action3 (azimuth 0) should have weight 0 when blend is at minimum",
  );
});
...
test("setBlend: should interpolate correctly between two adjacent actions", () => {
  ...
  blendTree.invokeSetInfluence(1);
  blendTree.setBlend(1, 0.3); // 30% between action1 and action2
...
test("setBlend: should work with negative and arbitrary values", () => {
  ...
  blendTree.invokeSetInfluence(1);
  blendTree.setBlend(1, 7.5); // Between 0 and 15
...
test("setBlend: should transition from exact match to interpolation correctly", () => {
  ...
  blendTree.setBlend(1, 1.3); // 30% between actions 2 and 3
  assertEqualWithTolerance(
    action1.action.weight,
    0,
