...
test("precomputeTriangle: should correctly save origin", () => {
  const result1 = precomputeTriangle(
    { x: 0, y: 0 },
    { x: 1, y: 0 },
    { x: 0, y: 1 },
  );
  assertEqualWithTolerance(result1.origin.x, 0, "origin.x");
  assertEqualWithTolerance(result1.origin.y, 0, "origin.y");
});
